...
class PreferenceValidator {
  ...
  static validatePartial(update: Partial<UserPreferences>): Partial<UserPreferences> {
    return PreferenceSchema.partial().parse(update);
  }
}
...
export { PreferenceValidator, PreferenceValidationError, type UserPreferences };typescript
interface UserPreferences {
  theme: 'light' | 'dark' | 'auto';
  notifications: boolean;
  language: string;
