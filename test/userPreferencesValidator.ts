...
class UserPreferencesValidator {
  ...
  private static validateFontSize(size?: number): number {
    ...
    const clampedSize = Math.max(
      this.MIN_FONT_SIZE,
      Math.min(this.MAX_FONT_SIZE, Math.round(size))
    );

    if (clampedSize !== size) {
      console.warn(`Font size ${size} clamped to ${clampedSize}`);
    }
...
function displayValidationResult(prefs: Partial<UserPreferences>): void {
  try {
    const validated = validateUserPreferences(prefs);
    console.log('Valid preferences:', validated);
  } catch (error) {
...
export { UserPreferences, PreferenceError, validateUserPreferences, displayValidationResult };import { z } from 'zod';
