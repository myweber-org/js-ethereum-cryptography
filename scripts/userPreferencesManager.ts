...
class UserPreferencesManager {
  ...
  private validatePreferences(data: unknown): UserPreferences {
    ...
    return {
      theme: this.isValidTheme(prefs.theme) ? prefs.theme as 'light' | 'dark' : defaultPrefs.theme,
  ...
  hasValidPreferences(): boolean {
    ...
    return current.theme === 'light' || current.theme === 'dark' &&
           typeof current.notifications === 'boolean' &&
           typeof current.language === 'string' &&
           typeof current.fontSize === 'number' &&
           current.fontSize >= 8 && current.fontSize <= 24;
  }
}
