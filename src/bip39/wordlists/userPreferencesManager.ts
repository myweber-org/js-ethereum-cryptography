...
class PreferencesManager {
  ...
  private validatePreferences(data: unknown): UserPreferences {
    ...
    if ('itemsPerPage' in data && typeof data.itemsPerPage === 'number') {
      const items = Math.max(10, Math.min(100, data.itemsPerPage));
...
export class UserPreferencesManager {
  ...
  constructor() {
    ...
    this.preferencesSubject = new BehaviorSubject(saved);
  ...
  private validatePreferences(data: unknown): UserPreferences {
    ...
    if ('itemsPerPage' in data && typeof data.itemsPerPage === 'number' && data.itemsPerPage > 0) {
      validated.itemsPerPage = Math.min(data.itemsPerPage, 100);
    }
