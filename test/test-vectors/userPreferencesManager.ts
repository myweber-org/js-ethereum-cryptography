...
class PreferencesManager {
  ...
  hasUnsavedChanges(current: Partial<UserPreferences>): boolean {
    return Object.keys(current).some(key => {
      ...
      return this.preferences[typedKey] !== current[typedKey];
    });
  }
...
class UserPreferencesManager {
  ...
  updatePreferences(updates: Partial<UserPreferences>): UserPreferences {
    ...
    const changed = Object.keys(this.preferences).some(
      key => this.preferences[key as keyof UserPreferences] !== previousPreferences[key as keyof UserPreferences]
    );
