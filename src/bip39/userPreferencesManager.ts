...
class UserPreferencesManager {
  ...
  applyToDOM(): void {
    document.documentElement.setAttribute('data-theme', this.preferences.theme === 'auto' ? 
...
class UserPreferencesManager {
  ...
  updatePreferences(updates: Partial<UserPreferences>): void {
    ...
    try {
      ...
      this.notifyChange(previous, this.preferences);
    } catch (error) {
      console.error('Failed to save preferences:', error);
  ...
  private notifyChange(previous: UserPreferences, current: UserPreferences): void {
    const changes = Object.keys(current).filter(
      key => previous[key as keyof UserPreferences] !== current[key as keyof UserPreferences]
    );

    if (changes.length > 0) {
      window.dispatchEvent(
        new CustomEvent('preferencesChanged', {
          detail: { changes, previous, current }
        })
      );
    }
  }
