...
class UserPreferencesManager {
  ...
  isAccessible(): boolean {
    return this.preferences.fontSize >= 16;
  }
}
...
export const preferenceManager = new UserPreferencesManager(defaultPrefs);
interface UserPreferences {
  theme: 'light' | 'dark' | 'auto';
  notifications: boolean;
  fontSize: number;
...
function demonstrateUsage(): void {
  const manager = new PreferencesManager({ theme: 'dark', fontSize: 18 });
  
  console.log('Initial preferences:', manager.getPreferences());
  
  manager.updatePreferences({ language: 'fr-FR', notifications: false });
  console.log('Updated preferences:', manager.getPreferences());
  
  const exported = manager.exportAsJSON();
  console.log('Exported JSON:', exported);
  
  const importedManager = PreferencesManager.importFromJSON(exported);
  console.log('Imported preferences:', importedManager.getPreferences());
  
  manager.resetToDefaults();
  console.log('After reset:', manager.getPreferences());
}