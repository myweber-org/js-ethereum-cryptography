...
class UserPreferencesManager {
  ...
  private validatePreferences(data: any): UserPreferences {
    ...
    return {
      ...
      language: typeof data.language === 'string' && data.language.length > 0 ? data.language : defaults.language,
...
class UserPreferencesManager {
  ...
  private mergeWithDefaults(partial: Partial<UserPreferences>): UserPreferences {
    return {
      ...DEFAULT_PREFERENCES,
      ...partial,
      fontSize: Math.max(MIN_FONT_SIZE, Math.min(MAX_FONT_SIZE, 
        partial.fontSize ?? DEFAULT_PREFERENCES.fontSize))
    };
  }
}
