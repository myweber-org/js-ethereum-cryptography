...
class UserPreferencesManager {
  ...
  private notifyPreferencesChange(oldPrefs: UserPreferences, newPrefs: UserPreferences): void {
    ...
    if (oldPrefs.notificationsEnabled !== newPrefs.notificationsEnabled) {
      const oldState = oldPrefs.notificationsEnabled ? 'enabled' : 'disabled';
      const newState = newPrefs.notificationsEnabled ? 'enabled' : 'disabled';
      changes.push(`notifications ${oldState} to ${newState}`);
    }
