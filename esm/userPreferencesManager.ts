...
class UserPreferencesManager {
  ...
  private loadPreferences(initial?: Partial<UserPreferences>): UserPreferences {
    try {
      const stored = localStorage.getItem(UserPreferencesManager.STORAGE_KEY);
      if (stored) {
        const parsed = JSON.parse(stored);
        return UserPreferencesSchema.parse({ ...parsed, ...initial });
      }
    } catch (error) {
      console.warn('Failed to load stored preferences:', error);
    }
    
    return UserPreferencesSchema.parse(initial || {});
  }
